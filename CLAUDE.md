...
## Развесовка карта (Balance, `/balance/`, реализовано 2026-08-31, доработано 2026-09-01)
...
- **Блок 10 — аналитика (§10).** Счётчик Яндекс.Метрики (тот же id `108405016`, что на gripline.ru — §10.1 требует общий) добавлен в `balance/app_base.html`, `window.__balanceYmCounter`. События через `ym(...,'reachGoal',...)`: `calc_completed`, `ballast_moved`, `diagnostic_link_clicked`. `save_attempt_anonymous`/`setup_saved`/`registration_from_tool` — ждут Блок 3.
- **Блок 5 — частично.** Селектор погоды (`dry/wet/mixed`) в UI → питает правило `wet_weather` в `diagnose()` (раньше было «неоткуда взять значение»). Трасса, авто-подтягивание температуры, ручной ввод температуры — ждут Блок 3 (без сохранения контекст сетапа некуда девать).

**НЕ сделано (каждый — самостоятельный объём работ):**
- **Блок 3 — авторизация/сохранение/шеринг/копирование/список сетапов.** Yandex OAuth **уже полностью реализован** в проекте (`social_django` + `accounts/backends.py::GriplineYandexOAuth2` + pipeline + онбординг-вью; нужен только `SocialAuthSettings.yandex_enabled` + креды в админке) — предупреждение прошлого чекпоинта про «требует Yandex OAuth» устарело, блок разблокирован. Модели `Kart`/`Setup`/`Ballast`/`TeamRosterEntry` (миграции 0037-0040) готовы. Нужно: вью сохранения (POST из калькулятора), страница «Мои сетапы» (таб в `app_base.html` сейчас `--disabled`), права (`Driver` видит свои + расшаренные командой; `TeamManager` — все сетапы команды), шеринг-флажки, копирование (`parent_setup`), мягкое архивирование, rate limiting по IP на серверные эндпоинты (§12).
- **Блок 7 — сравнение сетапов + офлайн.** Зависит от 3+4+5. Режим «два сетапа рядом» + дельты; реальный service worker (кэш shell/статики) + IndexedDB-очередь + фоновая синхронизация + диалог при конфликте. `balance-sw.js` сейчас — заглушка install/activate.
