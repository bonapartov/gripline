...
| `RaceClass` | Класс карта. Поле `sort_order` задаёт порядок по регламенту дисциплин (не алфавит) везде на сайте — см. «Порядок классов картинга» ниже |
...
**Кэш и owner-гейт (нашли и исправили на проде 2026-08-30, до продакшн-инцидента не было видно локально):** `wagtailcache` подключён глобальным middleware (`UpdateCacheMiddleware`+`FetchFromCacheMiddleware`, `mysite/settings/base.py`) — кэширует ЛЮБОЙ ответ по URL, без учёта пользователя/сессии. Без `@nocache_page` на `driver_mediakit_pdf_view` первый ответ на `/mediakit/` (даже 403 анонимному) закэшировался бы и отдавался всем следующим посетителям в обход owner-гейта. `WAGTAIL_CACHE = False` в `dev.py` — поэтому баг не проявлялся ни разу при разработке, нашёлся только на первом реальном прод-деплое. **Правило на будущее: любой персонализированный/owner-gated Django-view (не Wagtail Page.serve) в этом проекте обязан быть декорирован `@nocache_page` (`from wagtailcache.cache import nocache_page`)** — иначе кэш молча пробивает авторизацию.

---

## ЛК команды: агрегация, highlights, история, медиа-кит (реализовано 2026-08-30)

Развитие страницы команды (`/teams/<slug>/`, `team_detail_view` в `website/views.py`, шаблон `website/templates/coderedcms/snippets/team_page.html`) с минимума (список пилотов по классам) до аналога ЛК пилота — агрегированная статистика, highlights, история выступлений с drill-down «Ход гонки», медиа-кит для спонсоров. Полное ТЗ v1.0 — вне репозитория, у пользователя.

**Дизайн-ограничения ТЗ (раздел 2) содержали внутреннее противоречие, разрешено с пользователем:** текст требовал цветов `#E24B4A/#111111/#F0EEE8/#C8C4BC` как «существующих design tokens сайта» — но это цвета логотипа/печатной палитры (та же, что в `mediakit-print.css`), ни один не входит в реальные `--gl-*` токены тёмной темы сайта (`--gl-brand-yellow #ffc107`, `--gl-surface-card #1a1a25` и т.д.) и ни один не использовался на странице команды. Буквальное выполнение значило бы перекрасить тёмную страницу в светлую печатную палитру — что противоречило бы второму пункту того же раздела («сохранить текущую типографику страницы»). **Решение: новые блоки — в `--gl-*` токенах тёмной темы**, как остальная страница; печатная палитра остаётся только у PDF-медиа-кита.

**Период агрегации — вся история, без временного окна (подтверждено с пользователем):** ТЗ (раздел 5) просил переиспользовать «уже действующую логику 12 месяцев» блока деления пилотов по классам — но эта логика **не существовала**: `twelve_months_ago` в `team_detail_view` вычислялась и никогда не использовалась в фильтрации (мёртвый код, удалён), а текст в шаблоне про «за последние 12 месяцев» не соответствовал действительности (страница всегда показывала всю историю). Заодно поправлен вводящий в заблуждение текст алерта на «когда-либо выступали за эту команду».

**Файлы:**

| Файл | Роль |
|------|------|
| `website/services/highlights.py` | Общая лесенка highlights (`is_finished`, `stability_highlight`, `fallback_highlights`) — вынесена из `services/mediakit.py` при добавлении highlights команды, переиспользуется обоими |
| `website/services/team_stats.py` | Агрегация: `team_roster()`/`team_results()` (текущий ростер + его результаты ЗА эту команду), `compute_team_stats()`, `compute_team_highlights()` (лесенка команды), `compute_team_history()` (агрегация по этапам + группы для drill-down) |
| `website/services/team_mediakit.py` | Контекст медиа-кита команды — зеркало `services/mediakit.py`, переиспользует `team_stats.py` |
| `website/team_mediakit_views.py` | View медиа-кита команды + `_is_team_manager()` (owner-гейтинг через `teams.models.TeamManager`), `@nocache_page` |
| `website/templates/mediakit/team_mediakit.html` | Шаблон PDF команды — тот же движок/CSS/шрифты, что у пилота (`mediakit-print.css`, дополнен классами `.mk-roster-*` для секции «Ростер») |

**Скоуп результатов для статистики/highlights/истории — `RaceResult.team == эта команда`**, не «все результаты пилотов ростера когда-либо, за любую команду» — так уже считался блок «Пилоты по классам» на этой странице, для консистентности используется тот же скоуп. **Титул — исключение**: `_team_best_title()` берёт любой титул пилота ТЕКУЩЕГО ростера, даже выигранный до перехода в команду — титулы не привязаны к команде в модели данных (`ChampionshipPage.get_champions_by_class()` не хранит команду), это честное «у нас в составе есть чемпион», не приписывание чужого результата.

**Team highlights — та же лесенка, что у медиа-кита пилота, но тир 2 другой:** 1) Титул (см. выше) → 2) **лучший ПОДИУМ** (одна конкретная строка «место + этап + пилот», не счётчик — в отличие от пилотской лесенки, где тир 2 это счётчик подиумов; так явно требовал ТЗ раздел 6) → 3) Стабильность (переиспользует `AnalyticsSettings.mediakit_top10_min_field`, не дублирует) → 4) фолбэк (`fallback_highlights` — лучший результат и/или круг, тот же переиспользуемый код, что у пилота).

**История выступлений команды** (`compute_team_history()`) — агрегация по `EventPage` (не по пилоту): этап → участвовавшие пилоты → лучший результат команды (исключает DNF/DQ, тот же принцип что `best_result` в Career highlights пилота). **Ход гонки: multi-select виджета** — на одном этапе может быть несколько групп (разных классов), lap chart — per-group данные, поэтому на КАЖДУЮ группу с данными отдельный триггер, подсвечивающий ВСЕХ пилотов команды в этой группе разом (не одного).

**`gripline_lap_chart_widget.js` расширен до multi-select (обратно совместимо):** `options.initialSelected` теперь принимает либо один `race_number` (как раньше — driver/event страницы не менялись), либо массив (команда). Внутри `selected` — Set (`toSelectedSet()`), не скаляр. Клик по пилоту в виджете **всегда** переключает в одиночный режим (снимает мульти-выделение) — сохранена привычная ручная семантика клика. `lap_chart_modal.html` читает новый `data-race-numbers` (через запятую, приоритет) либо старый `data-race-number` (как раньше).

**Owner-гейтинг — `_is_team_manager(request, team)`** (`teams.models.TeamManager.objects.filter(user=..., team=team, is_active=True)`), НЕ то же самое, что ЛК менеджера на `/teams/dashboard/` (`teams/views.py`, где уже был похожий паттерн проверки, повторявшийся ~10 раз inline) — эта функция специально для публичной страницы `/teams/<slug>/` (кнопка медиа-кита в хедере), там раньше подобной проверки не было вообще.

**Team.city — новое поле** (миграция `0036`), добавлено в `Team.panels` (Wagtail admin) и в `TeamForm`/`dashboard.html` (ЛК менеджера, `teams/` приложение) — источник «города/базы» для хедера медиа-кита команды, аналог `Driver.city`, но для команды такого поля не было вообще.

**Баги, найденные и исправленные попутно (раздел 4 ТЗ + смежные, тем же вечером):**
1. `{{ team.description|safe|default:"…" }}` — **порядок фильтров** рендерил буквальный текст `None` на проде вместо плейсхолдера: `safe` на `None` даёт `mark_safe(None)` → строка `"None"` (непустая, truthy) ДО того, как отработает `default`. Исправлено на явный `{% if team.description %}…{% else %}CTA{% endif %}` (не просто перестановку фильтров — заодно добавлен CTA «Это ваша команда? Войдите…», раздел 4 ТЗ).
2. «Информация о руководителе отсутствует» — бага не было (обычный `{% if %}`), но добавлен CTA рядом, как и просил раздел 4.
3. Мёртвый код `twelve_months_ago` — см. выше.

---

## Развесовка карта (Balance, `/balance/`, реализовано 2026-08-31, доработано 2026-09-01, Блок 3 — 2026-09-10, Блок 7 — 2026-09-18)

Калькулятор развесовки карта — по ТЗ v0.3 (`~/Загрузки/gripline_tz_razvesovka_v03.md` у пользователя; отдельно `~/Загрузки/gripline_tz_yandex_oauth.docx` — вход через Яндекс). Идея вдохновлена сторонним продуктом KartBalance (kartbalance.com) — реализация с нуля, без копирования кода/интерфейса/текстов; формулы и целевые диапазоны общеотраслевые.

Минимальный публичный запуск по ТЗ (§14: блоки 0–4 + 6 + 8 + 11) — **реализован полностью**.

**Доработка 2026-09-01:**
- **Единая схема карта (§3.4 + §5.1).** Раньше Блок 4 не переверстал схему Блока 2 под drag-and-drop, а пририсовал **вторую** копию контура карта ниже — пользователь это заметил. Сведено в один SVG (`#glCalcSchemaSvg`, viewBox 280×400): угловые поля, зоны балласта, маркеры грузов и маркеры CG на одном контуре. Итоговый вес угла (база + Δ от грузов, §5.3) показывается строкой `→ N.N` под полем ввода. Без геометрии слой балласта заблокирован (`.gl-calc-schema--ballast-locked` + подсказка внутри контура), но Блок 2 (веса + метрики) работает как раньше. Геометрия (база/колея) поднята в блок `.gl-calc-setup` под выбор класса.
- **Блок 9 — deep-link из статей (§11.3).** `applyDeepLinkParams()` в `balance-calculator.js`: `?lf=&rf=&lr=&rr=` + опционально `&class=<id|slug>`, `&wb=&tf=&tr=` (геометрия), `&weather=dry|wet|mixed`. Только клиент, ничего не сохраняется. **Промо-блоки на главной/в Матчасти (§11.1-11.2) — НЕ сделаны** (нужен `BaseGenericSetting` + миграция + вёрстка в двух CSS-контекстах).
- **Блок 10 — аналитика (§10).** Счётчик Яндекс.Метрики (тот же id `108405016`, что на gripline.ru — §10.1 требует общий) добавлен в `balance/app_base.html`, `window.__balanceYmCounter`. События через `ym(...,'reachGoal',...)`: `calc_completed`, `ballast_moved`, `diagnostic_link_clicked`. `save_attempt_anonymous`/`setup_saved`/`registration_from_tool` — с появлением Блока 3 технически возможны, события пока не расставлены по коду сохранения (см. «НЕ сделано» ниже — не входило ни в план Блока 3, ни в задачи по нему).

**Блок 3 — авторизация/сохранение/шеринг/копирование/список сетапов (реализовано 2026-09-10, коммит `9ba92c1`, план `/home/v/.claude/plans/smooth-snacking-spindle.md`).** Yandex OAuth уже был полностью реализован в проекте (`social_django` + `accounts/backends.py::GriplineYandexOAuth2` + pipeline + онбординг-вью). Модели `Kart`/`Setup`/`Ballast`/`TeamRosterEntry` (миграции 0037-0040). Права — `website/services/balance_permissions.py` (единственное место с логикой видимости/редактирования, никогда не инлайн): `visible_karts_for_user`/`visible_setups_for_user`/`can_edit_kart`/`can_edit_setup`; лимиты и rate-limit — `website/services/balance_limits.py`. Эндпоинты (`website/balance_setup_views.py`): `kart_create`/`kart_archive` (только пилотский карт — командный из `teams/balance_roster_views.py::team_kart_save`), `setup_save` (create+update одним эндпоинтом, различаются по `setup_id` в теле; `kart_id` при update игнорируется — карт сменить нельзя), `setup_edit` (рендерит тот же `home.html`, не отдельный шаблон), `setup_copy` (только владелец — получателю шера копия была бы бессмысленна; сброс полей на дефолты — погода/температура/шеринг/`is_archived`), `setup_archive`, `setup_share` (валидирует применимость цели шеринга к типу владения картом), `setups_list` (`/balance/mine/`, весь датасет одним JSON-блобом, фильтрация/группировка по карту на клиенте). Вход в `/balance/mine/` — через уже существующую `/choose-role/` (кнопка «Мои сетапы» без доступа → модалка с переходом), не прямой Yandex OAuth в обход неё; свой GET-параметр `balance_next` для возврата после логина, не `next` (тот резервирует `python-social-auth`, см. learning `psa-next-param-collision`). 57+ тестов (`website/tests/test_balance_setups.py`, `teams/tests.py`).

**Блок 7 — сравнение сетапов реализовано (2026-09-18, `/balance/compare/`), офлайн-режим — ещё не начат.** `website/services/balance_calc.py::compare_setups()` — дельты по угловым весам и всем метрикам (front/rear, left/right, cross weight, итого, карт без пилота), подсветка значимых изменений (дельта ≥ 0.5 кг/п.п. ИЛИ пересечение границы классификации green/yellow/red — маленькое число, пересёкшее зону, тоже значимо). Бонусный сценарий ТЗ («как менялась развесовка при переходе из Юниора в Макс») — баннер при `same_kart and class_changed`. Вход — чекбоксы «Сравнить» на карточках `/balance/mine/` (ровно 2, плавающий бар) → `/balance/compare/?a=&b=`; страница сравнения одновременно работает как пикер (два `<select>`, group by карт) для прямого захода/смены одной стороны. Права — та же `visible_setups_for_user`; недоступный/несуществующий id тихо игнорируется (мягкая деградация, не 404 — read-only страница выбора из своего же списка). Офлайн-часть Блока 7 (реальный service worker с кэшем shell/статики, IndexedDB-очередь, фоновая синхронизация, диалог при конфликте) — **не сделана**, `balance-sw.js` всё ещё заглушка install/activate.

**НЕ сделано (каждый — самостоятельный объём работ):**
- Промо-блоки калькулятора на главной/в Матчасти (§11.1-11.2) — нужен `BaseGenericSetting` + миграция + вёрстка в двух CSS-контекстах.
- Блок 5 — трасса/авто-подтягивание температуры/ручной ввод температуры (селектор погоды `dry/wet/mixed` уже работает). Раньше блокировалось отсутствием сохранения контекста сетапа — с Блоком 3 это больше не преграда, просто ещё не взялись.
- Блок 7 — офлайн-режим (см. выше).
- Метрики `save_attempt_anonymous`/`setup_saved`/`registration_from_tool` (Блок 10) — теперь технически возможны (Блок 3 сохраняет сетапы), но `ym(...,'reachGoal',...)` для них ещё не расставлены в `balance-setup-editor.js`/`balance_setup_views.py`.
- Справочники: минимальные веса классов и геометрия по регламенту РАФ 2026, ссылки на статьи Матчасти для диагностических правил — таблицы пустые, заполняются через админку «Развесовка», когда будут реальные цифры.

### Архитектура

- **Роутинг** — простой Django `path('balance/', include('website.balance_urls'))` в `mysite/urls.py`, НЕ Wagtail Page и НЕ отдельный Wagtail Site (`django-hosts` не установлен и не нужен — единственный домен). Модели — в `website/models.py` (правило проекта «единое приложение»), views — отдельный модуль `website/balance_views.py` (по образцу `mediakit_views.py`, не раздувать `views.py`).
- **App-shell** (`website/templates/balance/app_base.html`) — без сайтового header/footer, `100dvh`, `viewport-fit=cover`, `env(safe-area-inset-*)`, нижняя таб-навигация. PWA: `manifest.json` + минимальный service worker (`website/static/website/js/balance-sw.js`, только install/activate/no-op fetch — без него Android не предложит установку вообще, реальное офлайн-кэширование — Блок 7) + баннер «Добавить на экран» после 3 визитов (localStorage-счётчик), с отдельной веткой для iOS (нет `beforeinstallprompt`, показывается инструкция).
- **Расчётный слой дублирован сознательно** (ТЗ §15): `website/services/balance_calc.py` — канонический Python (тесты, серверный рендер, Блок 6 диагностика), `website/static/website/js/balance-calculator.js` — та же математика в JS для расчёта в реальном времени без похода на сервер (ТЗ §3.4). Паритет проверен вручную через Node на тех же контрольных значениях, что в pytest — при правке формулы менять оба места синхронно, иначе они разойдутся незаметно.
- **Справочники — не хардкод, а админка** (пользователь явно попросил: «нужен отдельный пункт в меню админки, в котором будут вноситься данные, которые не захардкожены»). Пункт **«Развесовка»** в боковом меню Wagtail admin (`website/wagtail_hooks.py::BalanceGroup`): `ChassisTypePreset` (геометрия по умолчанию, 2 записи junior/senior), `KartClass` (классы: мин. вес, геометрия с переопределением), `BalanceThreshold` (пороги цветовой индикации, 3 метрики), `BalanceDiagnosticRule` (диагностический текст Блока 6 + обучающий блок). Все графики/формулы читают эти таблицы через `website/services/balance_calc.py::load_thresholds()`/`load_diagnostic_rules()` — не через отдельные захардкоженные числа, чтобы админ-правка порога и диагностический текст не расходились между собой.

### Модели (`website/models.py`, миграции 0037–0040)

Справочники (Wagtail admin): `ChassisTypePreset`, `KartClass`, `BalanceThreshold`, `BalanceDiagnosticRule` (текст 8 правил предзаполнен из ТЗ миграцией-данных `0040`, ссылки на статьи Матчасти — пустые, см. «Открытые вопросы ТЗ» ниже).

Пользовательские данные (обычный `django-admin`, не CMS — тот же паттерн, что `TeamManager`/`TeamClaim` в `teams/admin.py`): `TeamRosterEntry`, `Kart`, `Setup`, `Ballast`. `Kart` — DB `CheckConstraint` «ровно один владелец» (пилот **или** команда, не оба и не ни одного) — проверено и на уровне `clean()`, и на уровне PostgreSQL напрямую. `Setup.clean()` не даёт сохранить класс, не подходящий типу шасси карта.

### Формулы (`website/services/balance_calc.py`)

- Блок 2 (§3.1-3.5): `compute_metrics`, `validate_corners`, `classify_all`, `min_weight_status`, `evaluate_setup`. Открытый вопрос ТЗ №13 (границы правдоподобия углового веса) закрыт эвристикой из самого ТЗ — предупреждение при угле выше половины `min_weight_kg` класса; без выбранного класса проверка просто не включается (не выдумывать глобальный порог).
- Блок 4 (§5.3-5.4, визуальный редактор): `ballast_deltas`, `apply_ballasts`, `center_of_gravity`, `target_center_of_gravity`. `x_front`/`x_rear` в формуле ТЗ — не два разных измерения, а один и тот же offset от левого края, делённый то на `track_front`, то на `track_rear`. Целевая точка CG (43/57+50/50) не захардкожена — середина зелёной зоны `BalanceThreshold`, с фолбэком на буквальные числа ТЗ, если пороги ещё не заведены.
- Блок 6 (§7.1, диагностика): `diagnose()` переиспользует УЖЕ настроенную классификацию green/yellow/red вместо отдельных чисел 42/44/48/52/49/51 — порог и диагностика двигаются синхронно. Нюанс ТЗ «Cross % вне 49.5–50.5 при нормальном L/R» — L/R не красный; если L/R сам красный (асимметрия по бортам), `cross_diagonal` не срабатывает, срабатывают `cross_mechanical`+`lr_asymmetry`. `wet_weather` — с 2026-09-01 срабатывает: селектор погоды `#glCalcWeather` добавлен в UI (Блок 5 частично), `currentWeather()` в `balance-calculator.js` отдаёт значение в `diagnose()`.

**57 тестов** (`website/tests/test_balance_calc.py`) — формулы Блоков 2/4/6, включая весь чек-лист ТЗ §3.6 (сумма Δ = масса груза, груз по центру → симметрично, груз над углом → вся масса туда, отрицательный груз, CG до/после добавления, нулевая геометрия, груз за пределами колеи).

### Тестовая инфраструктура (побочный, но нужный ремонт)

`pytest` в проекте был полностью нерабочим до этой сессии: `pyproject.toml` указывал на несуществующий пакет `gripline.settings.*` (реальный — `mysite`), `pytest-django`/`pytest-cov` не были установлены. Плюс реплей миграций `website` с нуля падал — `0013_add_qual_and_prefinal_fields` дублировала все 11 `AddField`, которые уже добавила `0010_add_session_type_and_timing_fields` (исправлено на `AlterField`, без новой миграции — на реальных БД ничего не меняет, там миграция уже отмечена применённой). Дальше по цепочке нашёлся ещё один похожий баг (дублирующийся `CreateModel` на `website_techarticleindexpage`) — **не чинили**, вне скоупа; обошли через `--no-migrations` в `addopts` (тестовая схема строится из текущих моделей, не через реплей 38 миграций).

### SEO (Блок 8, `website/schema.py`, `website/sitemaps.py`, `website/balance_views.py`)

`/balance/` — не Wagtail Page, поэтому не подхватывается автоматически ни `wagtail-seo` (`SeoMixin` только на Page), ни sitemap.xml (`wagtail.contrib.sitemaps` строит дерево только из `Page.objects.live()`) — оба вопреки прямым формулировкам ТЗ («используем уже установленный wagtail-metadata», «Wagtail генерирует sitemap штатно, /balance/ попадает автоматически»). OG/Twitter/canonical/JSON-LD `WebApplication` написаны вручную в `app_base.html`/`balance_views.py`; `website/sitemaps.py::BalanceSitemap` подмешан в `mysite/urls.py` рядом с Wagtail-сайтмапом через тот же `sitemaps={...}` механизм. `publisher` в JSON-LD — тот же облегчённый паттерн Organization (`name`+`url`), что уже используется как fallback-автор статьи (`ArticlePage.seo_struct_article_dict`) — единого объекта с `logo`/`sameAs` в проекте нет.

### Персональные данные (Блок 11, ТЗ §13)

**Страницы `/privacy-policy/` на сайте не было вообще** — ни в коде, ни в БД; ссылка в футере основного сайта (`templates/coderedcms/snippets/footer.html`) вела в 404 на проде. Создана через идемпотентную management-команду `python manage.py create_privacy_policy_page` (не data-migration — операции с деревом Wagtail-страниц через frozen-модели миграций Wagtail сам не рекомендует). Содержит честный минимальный каркас по всему сайту (не юридически полный документ по OAuth/Telegram/кукам — это на будущее, решение подтверждено с пользователем) + полный раздел про `/balance/` (состав данных, кто вносит, несовершеннолетние в ростере, сроки хранения, права, контакты). Ссылка — в футере инструмента (`app_base.html`, `<footer class="gl-balance-footer">`, раньше футера у `/balance/` не было вовсе).

`TeamRosterEntry` не хранит год рождения (152-ФЗ) — самый чувствительный элемент убран, только фамилия+имя+опциональная метка. Пометка «данные обрабатываются согласно политике» для формы добавления в ростер — сама форма ещё не существует (Блок 3, ЛК менеджера команды), текст готов заранее: `website/models.py::ROSTER_PRIVACY_NOTICE`.

### Дизайн — ТЗ трижды не совпало с реальными токенами/паттернами проекта

1. **Палитра.** ТЗ §1.5 выдавало `#E24B4A/#111111/#F0EEE8/#C8C4BC` за «существующие токены сайта» — на деле это цвета логотипа/брендбука (`logo/SVG/...`, untracked), не `--gl-*` тёмной темы. Тот же класс ошибки уже разбирался для ЛК команды (см. выше). Везде использованы реальные `--gl-*` (жёлтый акцент `#ffc107`, поверхности `#000000→#15151c→#1a1a25`).
2. **Кастомные дропдауны.** ТЗ утверждало «нативные `<select>` не используются — известное правило проекта». Проверено: неправда, почти везде на сайте именно стилизованный нативный `<select>` (`.gl-filter-select` на `driver_page.html`); единственный настоящий div-дропдаун — одноразовый ad-hoc код на `compare_drivers.html`, не переиспользуемый компонент. Использован стилизованный `<select>` (`.gl-calc-select`), не выдуман новый JS-компонент.
3. **wagtail-metadata.** ТЗ дважды (§1.5-эквивалент и §9.3) ссылалось на пакет `wagtail-metadata` — в проекте фактически `wagtail-seo` (уже фиксировалось при работе над schema.org, см. выше).

### Найденные и исправленные попутные баги

1. **Дубли `AddField` в миграции `0013`** — см. «Тестовая инфраструктура» выше.
2. **`ArticlePage.get_absolute_url()` не существует** — Wagtail Page использует `.url`, не Django-конвенцию `get_absolute_url()` (та — у `Driver`/`Team`/`Chassis`, не у Wagtail-страниц). Поймано живым 500-м при первой проверке привязки статьи к диагностическому правилу, не мимоходом.
3. **Блокировка ландшафтной ориентации (ТЗ §3.4) не работала ни разу с самого Блока 2.** CSS-селектор требовал `.gl-balance-content.gl-balance-orientation-sensitive` (оба класса на одном элементе), а по факту `gl-balance-orientation-sensitive` — на вложенном `<div>` внутри `<main class="gl-balance-content">`. Найдено не в браузере (проект не проверяется во фронтенде самостоятельно, см. ниже), а при повторном чтении вёрстки под Блок 8.
4. **`/privacy-policy/` в футере сайта вела в 404** — см. «Персональные данные» выше.

### Открытые вопросы ТЗ, оставленные открытыми сознательно

- **№10 (список статей Матчасти для диагностики)** — не блокировал реализацию. `BalanceDiagnosticRule.article` — пустой FK на `ArticlePage`, редактируется в «Развесовка» → «Диагностика», когда статьи определены/опубликованы; до тех пор диагностика просто не показывает «Подробнее →».
- **Геометрия по умолчанию, минимальные веса классов (регламент РАФ 2026)** — та же логика: `ChassisTypePreset`/`KartClass` пустые, заполняются через админку, когда будут реальные цифры.
- **Порог правдоподобия углового/балластного веса** — закрыт собственной эвристикой ТЗ (см. «Формулы» выше), отдельного решения не требуется.

---

## Порядок классов картинга — регламент вместо алфавита (2026-09-18)

Везде на сайте, где выводится список/фильтр классов (`RaceClass`), сортировка была по алфавиту — `RaceClass.Meta` не задавал `ordering`, вызывающий код добавлял `.order_by('name')`/`sorted(key=lambda x: x.name)` почти в 20 разных местах: `/pulse/`, `/championships/<slug>/`, `/top/drivers/`, `/compare/`, `/compare-drivers/`, `/matrix/` (chassis_track_matrix), weather-impact, tyre-analysis, главная (топ пилотов), страницы трассы/шасси/двигателя/пилота/команды, страница этапа (список участников, мини-таблица протокола), календарь событий (бейджи классов на карточках), ЛК менеджера команды, организаторская панель (список заявок), FastAPI `/classes` (мобильное приложение).

Новое поле `RaceClass.sort_order` (миграции `0041`/`0042`, значения по регламенту дисциплин, шаг кратен 10) + `Meta.ordering = ['sort_order', 'name']` — правится через панель класса в Wagtail-снипете «Классы гонок» (не хардкод). Порядок: Микро → Мини → Мини ГР-3 → Супер-мини → Супер-мини ГР-3 → ОК J → ОК → KZ2 → KZ2 Masters → RM (Micro/Mini/Junior/Senior/DD2/DD2 Masters) → 4Т (Дети/Юноши/Взрослые) → E-10 (БАМБИНИ/МИНИ).

`RaceClass.sort_key_map()` — `{название: sort_order}`, для мест, где группировка уже идёт по строке названия класса (не по объекту `RaceClass`) — агрегаты по пилоту/команде в `driver_detail_view`/`team_detail_view`/`team_ratings_view` (`website/views.py`).

**Отдельный скрытый случай, найденный не сразу:** фильтр-чипы на `/pulse/` рендерятся не из Django-контекста страницы, а клиентским `website/static/js/pulse.js` поверх JSON от `/api/v2/pulse/` (SPA-стиль) — там была своя `.localeCompare('ru')`-сортировка, первый (серверный) фикс её не касался никак, чипы на живой странице остались алфавитными. Добавлен `class_order` (`{имя: sort_order}`) в ответ `pulse_data()` (`website/api.py`), `pulse.js::_classSortValue()` использует его вместо `localeCompare` — и для чипов-фильтра, и для списка чемпионов сезона.

**Сознательно не тронуто:** порядок классов внутри `RaceClassResultGroup` (Wagtail `Orderable` — ручной порядок организатора в админке результатов, задаётся drag-and-drop, никогда не был алфавитным изначально, менять его означало бы отобрать существующую фичу); `website.services.balance_calc.KartClass` (отдельный справочник калькулятора развесовки — уже свой `sort_order`, не путать с `RaceClass`, см. docstring модели).

Коммиты: `856e035` (основной фикс), `1366d8f` (pulse.js/API).

## Карточки рейтинга на странице пилота — текущий класс первым (2026-09-18)

Вкладка «Рейтинг и аналитика» (`driver_page.html#tab-rating`) сортировала карточки классов по алфавиту (`_get_driver_class_ratings` в `website/views.py`, `class_ratings.sort(key=lambda x: x['class_name'])`). Теперь: класс с самой свежей датой последнего старта (`last_race_date`) — всегда первая карточка, остальные классы — следом в хронологическом порядке (от раннего к позднему). Класс без даты считается самым старым (не роняет сравнение на `None`). Второй цикл по `class_ratings` в том же шаблоне (JS-инициализация графиков «Динамика рейтинга») следует тому же порядку автоматически — правка только в Python. Блок «Текущая форма» на вкладке «Обзор» уже считал «самый свежий класс» независимо (`max` по `last_race_date`) и не менялся.

Коммит: `cd4459f`.

---

## Редизайн страницы пилота — вкладки «Обзор/Рейтинг/Соперники/История» (реализовано 2026-09-20)

Публичная страница пилота (`website/templates/coderedcms/snippets/driver_page.html`, `driver_detail_view` в `website/views.py`) переверстана по образцу DriverDB — компоновка хедер-полосой (аватар/имя/город + вкладки + действия справа) вместо прежней. Разработана в изолированной песочнице (`website/redesign_driver_lk/`, план `~/.claude/plans/fluffy-tinkering-aho.md`), проверена локально на реальных данных и одобрена пользователем, затем перенесена в боевой путь; песочница и временный роут `/drivers-redesign/<slug>/` удалены.

Новое на вкладках: «Профиль результатов» — радар Chart.js (рейтинг/% побед/% подиумов/% поулов/процентиль места, только реальные цифры пилота, без выдуманных сравнений), «Достижения карьеры» теперь показывает разом все применимые плашки (было — только одна лучшая), новая вкладка «Соперники» — 5 ближайших по месту в рейтинге текущего класса с кнопкой «Сравнить». Старый виджет «сосед сверху/снизу + немезис/жертва» внутри карточки рейтинга — вытеснен вкладкой «Соперники» и в новом шаблоне не рендерится (сами поля `driver_above`/`driver_below`/`nemesis`/`victim`/`gap_to_above`/`gap_to_below` в `_get_driver_class_ratings()` оставлены как есть — дешевле, чем менять публичный контракт функции ради неиспользуемого сейчас поля, могут понадобиться позже).

**Отрефакторено попутно:** формула Байесовского сглаживания (C=15) ранее была продублирована между `_get_driver_class_ratings()` (место/соседи пилота) и черновиком «Соперников» в песочнице (аналогичный цикл по всем пилотам класса). При переносе в боевой код вынесена в общий `_class_ranking_entries(class_id, all_drivers)` (`website/views.py`) — используется обеими функциями, а не переписывается второй раз.

**Найден и закрыт SEO-регресс до пуша в прод (не был виден на глаз при ревью в браузере):** черновой шаблон песочницы не переопределял блок `{% block struct_seo %}` — вместо `{{ schema_json_ld }}` (Person JSON-LD) + `{% breadcrumb_schema %}`, которые старый `driver_page.html` намеренно подставлял вместо дефолтного `wagtailseo/struct_data.html` (не работает на `driver` — это Django-модель, не Wagtail `Page` с `SeoMixin`), рендерился пустой `<script type="application/ld+json"></script>`. Проверено сравнением рендера черновика и живой страницы напрямую (`curl` + grep на `application/ld+json`) — не предположение. Блок `struct_seo` перенесён в новый шаблон дословно, JSON-LD восстановлен.

Статика/шаблон при переносе переименованы из временных `redesign_driver_lk`/`*_v2` в постоянные пути: `website/static/website/css/driver_page.css`, `website/static/website/js/driver_page.js`, `website/static/website/img/driver-placeholder.webp`.

Коммиты: `4e83f04` (песочница), следующий коммит в `git log` — перенос в боевой код и удаление песочницы.
