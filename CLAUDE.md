...
**Известное ограничение (не исправлено, не входило в объём задачи):** `_get_driver_career_highlights` при поиске «Титула» перебирает **все** `ChampionshipPage` × все их годы и каждый раз заново пересчитывает `get_champions_by_class(year)` (существующий метод, без кэша) — на реальных данных (2 чемпионата) страница пилота рендерится ~2с, что на порядок дороже остального контента страницы. Не оптимизировано осознанно — потребует либо кэширования на уровне `get_champions_by_class`, либо пересмотра его контракта, что выходит за рамки этой итерации.
