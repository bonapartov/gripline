...
**Движок — WeasyPrint, не headless Chromium.** Важное открытие при реализации: современный WeasyPrint (69.0) **больше не требует системных библиотек** (`libpango`/`libcairo`/`libgdk-pixbuf`/`libffi`) — начиная где-то в районе v53 он перешёл на собственный PDF-бэкенд (`pydyf`) вместо cairo. Изначальное опасение ТЗ («ставить apt-либы и на проде, и локально») **не подтвердилось** — `pip install weasyprint` на голом Ubuntu без единой системной либы уже рендерит PDF с кириллицей. Деплой не отличается от обычного `deploy.sh` (git pull + collectstatic + migrate + restart), никаких дополнительных шагов не требуется.
