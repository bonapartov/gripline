...
Запускается **вручную** после каждого ввода новых результатов соревнований. Вторая команда — кэш турнирных таблиц чемпионатов (`ChampionshipPage.standings_cache`), используется Career highlights на странице пилота и `/api/v2/pulse/`, см. раздел «Реструктуризация страницы пилота» ниже.
...
**Оставшееся узкое место (не исправлено, отдельная задача):** `_get_driver_track_records` перебирает **все** `RaceResult` сайта на каждый рендер страницы пилота (ищет рекорды трасс) — после фикса выше это самая тяжёлая часть страницы (~1.2с из ~1.5с общего времени, замер профайлером 27.08.2026). Тот же класс проблемы, тот же напрашивающийся фикс (материализация в БД вместо пересчёта на лету) — не сделано, ждёт решения пользователя.
