...
from .models import Ad, AdCategory, AdResponse
from django.db.models import Q
from django.contrib import messages
from django.shortcuts import render, redirect, get_object_or_404
from django.urls import path
from wagtail.snippets.views.snippets import SnippetViewSet
from website.models import Driver, Team, RaceResult, RaceClass, Track, Chassis, TyreBrand, TyreType, Tyre, Engine
...
def team_detail_view(request, slug):
    ...
    for driver in team_drivers:
        ...
        all_results = RaceResult.objects.filter(
            team=team,
            driver=driver
        ).select_related('group__race_class', 'group__page')
        ...
        # Для каждого класса определяем период выступлений
        for class_name, results in class_results.items():
            # Сортируем по дате
            results.sort(key=lambda x: x.group.page.last_published_at or x.group.page.first_published_at)
            
            first_date = results[0].group.page.last_published_at or results[0].group.page.first_published_at
            last_date = results[-1].group.page.last_published_at or results[-1].group.page.first_published_at
            
            # Проверяем, перешёл ли пилот в другой класс
            # Получаем все классы пилота, сортируем по дате последнего заезда
            all_driver_classes = {}
            for r in RaceResult.objects.filter(driver=driver).select_related('group__race_class'):
                class_name_driver = r.group.race_class.name
                if class_name_driver not in all_driver_classes:
                    all_driver_classes[class_name_driver] = []
                all_driver_classes[class_name_driver].append(r)
            
            # Находим самый поздний класс (текущий)
            latest_class = None
            latest_date = None
            for cls_name, cls_results in all_driver_classes.items():
                cls_results.sort(key=lambda x: x.group.page.last_published_at or x.group.page.first_published_at)
                cls_last_date = cls_results[-1].group.page.last_published_at or cls_results[-1].group.page.first_published_at
                if latest_date is None or cls_last_date > latest_date:
                    latest_date = cls_last_date
                    latest_class = cls_name
            
            # Формируем строку периода
            if class_name == latest_class:
                period = f"{first_date.strftime('%m.%Y')} — н/в"
            else:
                period = f"{first_date.strftime('%m.%Y')} — {last_date.strftime('%m.%Y')}"
            
            # Добавляем пилота в словарь с периодом
            if class_name not in driver_classes:
                driver_classes[class_name] = []
            
            # Проверяем, не добавлен ли уже пилот с таким классом
            existing = next((d for d in driver_classes[class_name] if d['driver'].id == driver.id), None)
            if not existing:
                driver_classes[class_name].append({
                    'driver': driver,
                    'period': period
                })
...
def compare_drivers_view(request):
    ...
    if class_id:
        ...
        print(f"[INFO] Выбран класс {class_id}, найдено пилотов: {filtered_drivers.count()}")
...
def ads_list(request):
    """Список объявлений"""
    ads_list = Ad.objects.filter(is_active=True)
    
    # Фильтры
    ad_type = request.GET.get('type')
    if ad_type:
        ads_list = ads_list.filter(ad_type=ad_type)
    ...
    return render(request, 'website/ads_list.html', {
        'ads': ads_list,
...
def ads_create(request):
    ...
    if request.method == 'POST':
        ...
        contact_telegram = request.POST.get('contact_telegram')
    ...
    categories = AdCategory.objects.all()
    return render(request, 'website/ads_form.html', {'categories': categories})


@login_required
def ads_respond(request, ad_id):
    """Отклик на объявление"""
    ad = get_object_or_404(Ad, id=ad_id)
    
    if request.method == 'POST':
        message = request.POST.get('message')
        AdResponse.objects.create(
            ad=ad,
            author=request.user,
            message=message
        )
        messages.success(request, 'Отклик отправлен! Автор объявления свяжется с вами.')
        return redirect('ads_detail', ad_id=ad.id)
    
    return render(request, 'website/ads_respond.html', {'ad': ad})
...
def ads_edit(request, ad_id):
    ...
    if request.method == 'POST':
        ...
        ad.category_id = request.POST.get('category')
        ad.title = request.POST.get('title')
        ad.description = request.POST.get('description')
        ad.price = request.POST.get('price') or None
        ...
        ad.contact_telegram = request.POST.get('contact_telegram')
        ad.save()
        messages.success(request, 'Объявление обновлено!')
        return redirect('ads_detail', ad_id=ad.id)
    
    categories = AdCategory.objects.all()
    return render(request, 'website/ads_form.html', {'ad': ad, 'categories': categories, 'is_edit': True})
...
def ad_responses(request, ad_id):
    ...
    return render(request, 'website/ad_responses.html', {'ad': ad, 'responses': responses})

@login_required
def ads_favorite(request, ad_id):
    """Добавление/удаление из избранного"""
    from .models import AdFavorite
    ad = get_object_or_404(Ad, id=ad_id)
    fav, created = AdFavorite.objects.get_or_create(user=request.user, ad=ad)
    if not created:
        fav.delete()
    return redirect('ads_detail', ad_id=ad.id)
