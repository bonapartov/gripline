/*
 * Песочница редизайна ЛК пилота. Чисто статический файл — без Django-тегов
 * внутри (иначе он не был бы валидным JS для {% static %}). Все данные,
 * которые раньше рендерились инлайн через {% for cr in class_ratings %},
 * здесь читаются из data-* атрибутов, проставленных шаблоном на элементах,
 * и из уже существующего JSON-блока истории (driverHistoryData,
 * тот же паттерн, что и на живой странице driver_page.html).
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    // из driver_page_v2.css. Здесь только подсветка активного пункта по скроллу
