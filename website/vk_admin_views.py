"""
Admin-эндпоинты для кнопки «Отправить в VK» на странице статьи в Wagtail Admin.
Сама логика сборки/отправки сообщения — в website/vk.py.
"""
...
from .vk import VK_TEXT_LIMIT, get_vk_message_overhead, send_to_vk
...
def vk_status(request, page_id):
    ...
    return JsonResponse({
        ...
        "live": article.live,
        "posted_at": article.vk_posted_at.isoformat() if article.vk_posted_at else None,
        "can_publish": can_publish,
        "overhead_len": get_vk_message_overhead(article),
        "has_image": has_image,
        "teaser_limit": VK_TEXT_LIMIT,
        "auto_tags": [{"tag": t.tag, "emoji": t.emoji} for t in auto_tags],
    })


@login_required
def vk_send(request, page_id):
    """Публикация анонса статьи в сообщество VK. Синхронно, с полной серверной
    ...
    if article.vk_posted_at:
        seconds_since = (timezone.now() - article.vk_posted_at).total_seconds()
    ...
    try:
        send_to_vk(article, request.user)
    except Exception:
        return JsonResponse({"success": False, "message": "Ошибка отправки в VK — проверьте логи"}, status=502)

    return JsonResponse({"success": True, "message": "Опубликовано в сообществе VK"})
